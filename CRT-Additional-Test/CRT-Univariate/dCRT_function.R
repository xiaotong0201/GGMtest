...
d_CRT <- function(X, Y, Sigma_X = NULL, FDR = 0.1, candidate_set = 1:length(X[1,]), 
...
Creat_condition_gaussian <- function(X, indx, Sigma = NULL, lambda = NULL){
  p=ncol(X)   # Missing line from the source code
  if (is.null(Sigma)){
    ...
    X_bar <- X[ ,-indx,drop=FALSE] %*% beta_x  #non-conformable arguments(可能因为矩阵退化成vector)
            #X[ ,-indx] 
